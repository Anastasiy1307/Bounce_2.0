...
public class Collisions {
    ...
    public void checkingCollision(Ball ball, float dt) {
        int begin_i = Math.min(Math.max((int)(ball.getRectangle().x) / 55 - 2, 0), Loader.map.length - 1);
        int end_i = Math.max(Math.min((int)(ball.getRectangle().x) / 55 + 2, Loader.map.length - 1), 0);
        int begin_j = Math.min(Math.max((int)(ball.getRectangle().y) / 55 - 2, 0), Loader.map[0].length - 1);
        int end_j = Math.max(Math.min((int)(ball.getRectangle().y) / 55 + 2, Loader.map[0].length - 1), 0);
        for (int i = begin_i; i <= end_i; i++) {
            for (int j = begin_j; j <= end_j; j++) {
                if (Loader.map[i][j] != null) {
                    ...
                    if (Loader.map[i][j] instanceof ExtraLife && Loader.map[i][j].getRectangle().overlaps(ball.getRectangle())) { Loader.numberOfLives++; Loader.map[i][j] = null;}
                }
            }
        }
    ...
    private void collisionRectangle(MainObject object, Ball ball, float dt) {
        if (object.getRectangle().overlaps(ball.getRectangle())) {
    ...
    private void collisionDeath(MainObject object, Ball ball) {
        if (object.getRectangle().overlaps(ball.getRectangle())) {
            ...
            ball.getRectangle().x = Loader.ballPosition.x;
            ball.getRectangle().y = Loader.ballPosition.y;
    ...
    private void collisionRise(MainObject object, Ball ball) {
        if (object.getRectangle().overlaps(ball.getRectangle()) && ball.getCircle().radius < 28) {
